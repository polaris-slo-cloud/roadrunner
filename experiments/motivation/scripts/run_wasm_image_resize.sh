...
RESULTS_FILE="${RESULTS_DIR}/results-wasm.csv"
...
start_run=$(date +%s%3N)
sudo ctr -n k8s.io run --rm --runtime="${RUNTIME}" --net-host=true "$IMAGE" "$CONTAINER_NAME" "$BINARY_PATH"
end_run=$(date +%s%3N)
